  ...
  for year_line in year_lines[1:]:
    try:
      ...
      if len(year)>0:
        ...
        year=int(year)
        # project_link='https://www.gafspfund.org/'+str(project_link)
        # years.append(project_link)
        if year>=2015:
          years.append(year)
    except:
      pass
  return years
def get_project_urls(years):
#concatenate each year to this link
# for year in years:
  projects_url='https://www.gafspfund.org/projects?location=All&funding=All&supervising=All&field_project_status_target_id=All&call_for_proposal=2021'
...
def get_project_links(links):
  ...
  for link in links:
    try:
      project_link = re.findall('/index.+', str(link))
      if len(project_link)>0:
        project_link=str(project_link)
        project_link=project_link.replace('[\'/', '').replace('\">\']','')
        project_link='https://www.gafspfund.org/'+str(project_link)
        urls.append(project_link)
    except:
      pass
  #get the urls to each project site like below
  for url in urls:
...
def main():
  ...
  links=get_project_urls(years)
  get_project_links(links)
